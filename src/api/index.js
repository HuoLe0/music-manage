...
//根据歌曲id查询歌曲对象
export const songOfSongId =(id) => get(`song/selectById?id=${id}`);
//根据歌曲名获取歌曲对象
export const songOfSongName =(name) => get(`song/selectByName?name=${name}`);
//根据歌曲名获取歌曲对象
export const songLikeName =(name) => get(`song/selectLikeName?name=${name}`);
//查询所有歌曲
export const allSong =() => get(`song/selectAll`);
...
//============歌单的歌曲相关============
//根据歌单id查询歌曲列表
export const listSongDetail = (songListId) => get(`listSong/listSongOfSongListId?songListId=${songListId}`);
//给歌单增加歌曲
export const listSongAdd = (params) => post(`songList/addSong`,params);
//删除歌单的歌曲
